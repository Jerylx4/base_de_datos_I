...
-- 1.5.6 Consultas multitabla (Composición externa)
  ...
  SELECT d.nombre
  FROM "universidad_A".asignatura d
  WHERE d.id_profesor IS NULL;
 
  --6. Devuelve un listado con todos los departamentos que tienen alguna asignatura que no se haya impartido en ningún 
  --curso escolar. El resultado debe mostrar el nombre del departamento y el nombre de la asignatura que no
  --se haya impartido nunca.
  SELECT dep.nombre AS nombre_departamento, asi.nombre AS nombre_asignatura
  FROM "universidad_A".asignatura asi
  JOIN "universidad_A".profesor prof ON asi.id_profesor = prof.id_profesor
  JOIN "universidad_A".departamento dep ON prof.id_departamento = dep."ID"
  LEFT JOIN "universidad_A".alumno_se_matricula_asignatura am 
...
-- 1.5.7 Consultas resumen
  ...
  SELECT COUNT (*)
  FROM "universidad_A".persona e
  WHERE e.sexo = 'M' AND e.tipo = 'alumno';
 
  --2. Calcula cuántos alumnos nacieron en 1999.
  SELECT COUNT (*)
  FROM "universidad_A".persona e
  WHERE EXTRACT(YEAR FROM fecha_nacimiento) = 1999 AND e.tipo = 'alumno';
  ...
  FROM "universidad_A".departamento d
  JOIN "universidad_A".profesor p ON d."ID" = p.id_departamento
  GROUP BY d.nombre
